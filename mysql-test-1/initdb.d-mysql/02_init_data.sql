...
insert into "order" (account_id)
	    with recursive
	    seq as (select 0 as ordinal union all select ordinal + 1 from seq limit 5),
	    sample as (
	      select
		*
		from
...
insert into order_detail (order_id, product_id, units)
	    ...
	    "order" as (
	      select
		"order".id,
		floor((rand()*9 + 1)) items
		from "order")
select
  order_id,
  product_id,
  floor((rand()*9 + 1)) units
  from (
    select
      "order".id order_id,
...
update "order" set status = json_unquote(json_extract(json_array('new', 'processing', 'fulfilled'), concat('$[', floor(rand()*3), ']')));
...
update "order" set region = json_unquote(json_extract(json_array(
  'NORTHEAST',
  'MIDWEST',
  'SOUTH',
  'PLAINS',
