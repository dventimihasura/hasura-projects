...
- docker-compose.yaml :: sets up services for ~postgres~, ~mysql~, ~graphql-engine~, and ~data-connector-agent~
- metadata.json :: Hasura metadata with the connection information and the tracked tables, views, and relationships
- 01_init_model.sql :: SQL script to initialize the data model
- 02_init_data.sql :: SQL script to initialize the data
